...
impl Coord {
	...
	pub fn add(&self, vector: Vector) -> Result<Self, ()> {
		...
		if new_x < 0 || new_y < 0 || new_x >= self.max_dimension as isize || new_y >= self.max_dimension as isize {
			Err(())
		} else {
			Ok(Self { x: new_x as usize, y: new_y as usize, max_dimension: self.max_dimension })
		}
	}
}
