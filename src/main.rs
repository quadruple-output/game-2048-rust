...
fn main() {
  let args =
    App::new("2048 (Example in Rust)").author(AUTHOR)
                                      ...
                                      .about("This is a board game, conceptually based on \
                                              http://git.io/2048 ")
                                      .arg(Arg::with_name(ARG_VIEW_TYPE).short("d")
                                                                        .value_name("DISPLAY_TYPE")
                                                                        .help("Sets the display type. See \
                                                                               -l. Default is \"ncurses\"."))
                                      .arg(Arg::with_name(ARG_LIST_VIEW_TYPES).short("l")
                                                                              .help("Lists available \
                                                                                     display types and \
                                                                                     exits"))
                                      .arg(Arg::with_name(ARG_VERBOSITY).short("v")
                                                                        .multiple(true)
                                                                        .help("Sets level of verbosity on \
                                                                               stderr. Repeat up to 4 \
                                                                               times for more details"))
                                      .arg(Arg::with_name(ARG_BOARD_WIDTH).short("w")
                                                                          .value_name("WIDTH")
                                                                          .default_value("3")
                                                                          .help("Set the width of the Board"))
                                      .arg(Arg::with_name(ARG_BOARD_HEIGHT).short("h")
                                                                           .value_name("HEIGHT")
                                                                           .default_value("3")
                                                                           .help("Set the height of the \
                                                                                  Board"))
                                      .get_matches();
  ...
  if args.is_present(ARG_LIST_VIEW_TYPES) {
    ...
    if let Some(view_type) = match args.value_of(ARG_VIEW_TYPE) {
      ...
      let width = match args.value_of(ARG_BOARD_WIDTH).unwrap().parse() {
        Ok(n) => n,
        Err(_) => 3
      };
      let height = match args.value_of(ARG_BOARD_HEIGHT).unwrap().parse() {
        Ok(n) => n,
        Err(_) => 3
      };
